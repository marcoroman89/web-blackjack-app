...
helpers do
  ...
  def tie!(msg)
    @play_again = true
    @show_hit_or_stay_buttons = false
    @winner = "<strong>Sorry, it's a tie!</strong> #{msg}"
  end
end
...
post '/bet' do
  if params[:bet_amount].nil? || params[:bet_amount].to_i == 0
    @error = "Hmmm...You forgot to place a bet"
    halt erb(:bet)
  elsif params[:bet_amount].to_i > session[:player_pot]
    @error = "The bet amount cannot be greater then what you currently have ($#{session[:player_pot]})"
    halt erb(:bet)
  else #happy path
